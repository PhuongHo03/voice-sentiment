...
from app.ai.speaker_diarization import SpeakerDiarizationClient
...
class WhisperSpeechToTextClient:
    ...
    def transcribe(self, audio: bytes) -> list[dict]:
        ...
        try:
            ...
            # 3. Run Whisper ASR with stable VAD parameters (1000ms) for high-quality, context-aware transcription,
            # and enable word_timestamps to allow precise word-level turn boundary detection.
            ...
            # 4. Collect words and group them into clean speaker turns based on punctuation and conversational pauses
            raw_segments = []
            ...
            for segment in segments:
                ...
                for idx, word in enumerate(words_list):
                    ...
                    # Rule 1: Ends with punctuation (. ? !) and gap to next word > 0.15s (very fast turns)
                    # Rule 2: Or gap to next word > 0.8s (large pause representing turn change)
                    has_split = False
                    if idx + 1 < n_words:
                        next_word = words_list[idx + 1]
                        gap = next_word.start - word.end
                        
                        has_sentence_end = any(word_text.endswith(p) for p in (".", "?", "!"))
                        if has_sentence_end and gap > 0.15:
                            has_split = True
                        elif gap > 0.8:
                            has_split = True
                    ...
                    if has_split:
                        ...
                        if text_str:
                            raw_segments.append({
                                "text": text_str,
                                "start_seconds": round(current_words[0].start, 2),
                                "end_seconds": round(current_words[-1].end, 2),
                            })
                        current_words = []
                        
                # Flush any remaining words at the end of the segment
                if current_words:
                    text_str = " ".join(w.word.strip() for w in current_words)
                    text_str = re.sub(r'\s+', ' ', text_str).strip()
                    if text_str:
            ...
            logger.info(f"faster-whisper transcription completed. Found {len(raw_segments)} segments after word-level splitting.")
