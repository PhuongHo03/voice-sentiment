...
│   ├── nginx.conf        ← Reverse proxy cho frontend/backend; Prometheus API không public qua Nginx
│   └── prometheus.yml    ← Scrape config cho app metrics, MinIO và infra exporters
...
*   **Nginx Proxy (`9090`)**: Cổng ngõ vào (Gateway) duy nhất của hệ thống phục vụ Web UI và API backend; Prometheus chỉ hoạt động nội bộ, Admin UI lấy metrics qua `GET /api/admin/metrics`.
